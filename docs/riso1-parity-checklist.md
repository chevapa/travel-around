...
| Filter by season | `js/filters.js` (season checkboxes) | — | ❌ not ported — issue #143 |
| Filter by source (journal vs. research) | `js/filters.js` (source checkboxes) | — | ❌ not ported — issue #143 |
...
| Country badge, clickable to filter | `js/map.js` (`countryBadge`) | Index panel's Country tab (not from the card itself) | ⚠️ partial — issue #144 |
| Season badge, clickable to filter | `js/map.js` (`seasonBadge`) | — | ❌ not ported — issue #144 |
| "⚠ check before you go" warning line | `js/map.js` (`place.warn`) | `Frame.warn` exists in the model but isn't rendered on the card | ❌ not wired — issue #145 |
