...
| Filter by season | `js/filters.js` (season checkboxes) | — | ❌ not ported (no season UI in RISO1 yet) |
| Filter by source (journal vs. research) | `js/filters.js` (source checkboxes) | — | ❌ not ported |
...
| Country badge, clickable to filter | `js/map.js` (`countryBadge`) | Index panel's Country tab (not from the card itself) | ⚠️ partial — country isn't a clickable badge on the card yet |
| Season badge, clickable to filter | `js/map.js` (`seasonBadge`) | — | ❌ not ported |
| "⚠ check before you go" warning line | `js/map.js` (`place.warn`) | `Frame.warn` exists in the model but isn't rendered on the card | ❌ not wired |
...
| Live drive-time lookup (real routing API) | `js/map.js` (`fetchDriveTime`, OSRM) | Only the migrated static estimate; no live lookup | ❌ not ported |
...
-fix pass this checklist came out of, but it should be the next epic task,
not a footnote.

| Behaviour | Old site (file) | RISO1 | Status |
|---|---|---|---|
| Swipeable recommendation card (like/skip/save, drag + buttons) | `js/recommend.js` | — | ❌ not started |
| Recommendation scoring (context, history, preferences) | `js/recommendationEngine.js` | — | ❌ not started |
| First-time onboarding (no history yet) | `js/recommend.js` (`onboarding cards`) | — | ❌ not started |

## 4. Person / profile page

| Behaviour | Old site (file) | RISO1 | Status |
|---|---|---|---|
| Visited/discovered place counts, stats | `js/stats.js`, `js/statsEngine.js`, `js/profile.js` | The Contact Sheet covers "what have I printed" but there's no dedicated profile/stats screen | ❌ not ported |
