...
| HIGH 03 — saturated basemap competes with pins | `MapBase.tsx` + `lib/mapStyle.ts` (real-basemap addendum) | `src/lib/saturation.test.ts` › the map's own restyle tokens (ground, roads) are proven less saturated than every marker/print colour (pink, blue, yellow), from the literal hex values `lib/mapStyle.ts` applies. Real headless-Chromium screenshots in the real-basemap PR (#108) additionally show prints/clusters as the visually dominant elements against the restyled cream/greige terrain. |
